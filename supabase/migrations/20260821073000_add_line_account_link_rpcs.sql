-- Phase 4 LINE Login account-link operations.
-- Only trusted Edge Functions running as service_role may execute these RPCs.

create function public.create_line_link_session(
  p_user_id uuid,
  p_state_hash text,
  p_nonce_hash text,
...
begin
  if p_user_id is null
    ...
    or p_expires_at <= current_time
    or p_expires_at > current_time + interval '10 minutes'
  then
  ...
  -- A member may have only one live browser flow. Starting again invalidates
  -- abandoned state values without retaining plaintext OAuth parameters.
...
create function public.complete_line_account_link(
  p_state_hash text,
  p_nonce_hash text,
  p_line_user_id text,
  p_is_friend boolean
...
begin
  ...
  where session.state_hash = decoded_state_hash
    ...
    and session.expires_at >= current_time
  for update;
...
create function public.unlink_line_account(
  p_user_id uuid
)
returns text
language plpgsql
...
begin
  ...
  set status = 'unlinked'::public.line_account_link_status,
...
revoke all on function public.unlink_line_account(uuid)
from public, anon, authenticated, service_role;
...
grant execute on function public.unlink_line_account(uuid)
to service_role;

comment on function public.create_line_link_session(
  uuid,
  text,
  text,
  timestamptz
) is
  'Creates one short-lived hashed LINE Login session for an active member.';

comment on function public.complete_line_account_link(
  text,
  text,
  text,
  boolean
) is
  'Consumes a verified LINE Login session and atomically enforces one-to-one account linking.';

comment on function public.unlink_line_account(uuid) is
  'Marks an active member LINE account link unlinked without exposing its LINE identifier.';
