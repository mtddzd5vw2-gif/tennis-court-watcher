...
現在のMonitoring Policyでは、今日を含む直近15日間の土日・日本の祝日、8:00〜13:00について、連続60分以上の空き候補を取得します。通知条件は監視範囲外でも保存できますが、通常の平日や8:00〜13:00と重ならない時間帯には現在照合対象データがありません。監視時間と一部だけ重なる条件は実際の重複時間で最低連続時間を判定するため、最低30分の条件も取得済み候補と30分以上重なれば一致します。祝日は実際の日付の曜日で判定します。通知条件画面の案内と08:00〜13:00の初期値は2026-08-20に本番確認済みです。
...
5. Monitoring Policyと監視範囲外条件のUI表示 — 完了（2026-08-20）
6. 鹿児島βで使用する運用指標の決定
...
- 自動予約は実装していません。
- 会員DB、規約同意履歴、RLS、規約同意RPC、会員情報表示、退会処理、Phase 2の通知条件、Phase 3の利用者別メールqueue/worker、自動enqueue/dispatch、delivery feedback、unsubscribe / re-enable、90日retention cleanupは本番反映・production acceptanceまで完了しています。Phase 4の利用者別LINE通知は未実装です。migrationの適用状況と実DB RLS検証状況は対象環境ごとに確認してください。
