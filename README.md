...
| `LINE_NOTIFICATION_USE_ALLOWLIST` | 限定βまたはそこへのrollback時だけ`true`。private DB allowlistをenqueue、claim、送信直前に強制 |
| `LINE_NOTIFICATION_ALLOW_ALL` | 全会員向け本番運転時だけ`true`。canary、allowlistとの同時有効は拒否 |
...
`SUPABASE_SERVICE_ROLE_KEY` はmatching/enqueue stepだけ、`EMAIL_DELIVERY_WORKER_SECRET` と `LINE_DELIVERY_WORKER_SECRET` は各dispatch stepだけへ渡します。LINE rolloutは単一canary、最大20会員のprivate allowlist、全会員のいずれか1モードだけを許可し、enqueue、worker claim、送信直前再検証の3境界で同じ対象を強制します。LINE Messaging APIのtokenはSupabase Edge Function secretに留め、通常のavailability workflowへ渡しません。各通知stepは `continue-on-error: true` で、失敗してもavailability Artifact・commit・Pages公開を妨げません。
...
7. Phase 4の利用者別LINE通知 — account link、queue/worker、本番単一会員canary、既存GAS互換bridge、限定β、全会員向けproduction acceptanceまで完了
