...
| UC-08 | 2 | 会員 | 通知したい施設と利用時間を保存・変更・停止する |
...
| プライバシーに関する表示 | `legal/privacy.html` | 公開 | 1 | 正式版 `2026-09-01`。取得情報、匿名の日別導線集計、利用目的、委託先、第三者提供、保持・削除、開示等請求 |
...
β運用では、ログイン画面表示、`LINEではじめる` 押下、利用規約確認画面表示の3地点を、日本時間の日別件数だけで集計する。同じブラウザでは各地点を1日1回まで数え、アプリの集計テーブルには会員ID、LINEユーザーID、メールアドレス、IPアドレス、User-Agent、参照元URLまたは端末識別子を保存しない。数値は導線改善用の概算であり、個人の行動追跡や本人特定には使用しない。保持期間は最大400日とする。定義と運用確認方法は[匿名登録ファネル](./ANONYMOUS_FUNNEL_METRICS.md)を正とする。
...
- 利用者の権利または義務に重要な影響を与える改定では再同意を必須とし、active会員を `pending_terms` へ移行する。
...
- 通常曜日はISO 8601の6（土曜日）と7（日曜日）を保存し、日本の祝日は `include_holidays` で独立して保存する。
- `date_from` は対象開始日、`date_to` は対象終了日とする。`date_from = null` は開始日の下限なし、`date_to = null` は終了日の上限なしを表す。
- `start_time` / `end_time` は8:00〜13:00内の1時間境界とし、2時間以上の範囲を保存する。時刻は対象施設が属する地域の `regions.timezone` で評価する。
- 最低連続時間は60〜300分の60分単位とし、選択した時間帯を超えない。
- `is_enabled` は有効・一時停止状態を表し、子テーブル登録前の不完全な条件を誤って有効扱いしないよう初期値を `false` とする。
- DB上は施設または通常曜日が0件の不完全な条件を保存できる。UIでは施設1件以上と、土曜・日曜・祝日のいずれか1件以上を必須検証し、照合処理でも同じ条件を満たさない設定を無効として扱う。
...
空き通知UIは、active会員本人の設定だけをRLS配下で読み込み、「登録済み n / 5件」を表示する。会員は施設、任意の日付範囲、土曜・日曜・祝日の選択、8:00〜13:00内の開始・終了時刻、利用時間を入力する。曜日・祝日は1件以上、時間帯は1時間境界で最低2時間とし、利用時間は60、120、180、240、300分の60分刻み、初期値120分で、選択時間帯を超える値は選べない。5件では新規作成ボタンを無効化して既存設定の削除を案内し、削除後の再取得で4件以下になれば再び有効化する。新規フォーム開始時と保存直前にも件数を確認するが、編集・有効化・一時停止・削除は上限の影響を受けない。
...
現行のMonitoring Policyでは、直近15日間の土日・日本の祝日、8:00〜13:00について、スクレイパーが連続60分以上の空き候補を生成する。会員向け空き通知はこの取得範囲を上限とし、その中から対象日と時間帯を絞り込める。

日条件は「空き日のISO曜日が選択した土曜・日曜に一致する」または「`include_holidays = true` かつ空き日が日本の祝日である」のORで判定する。したがって平日の祝日は祝日選択で一致し、祝日の土曜日は土曜または祝日のどちらを選んでも一致する。祝日でない平日はMonitoring Policyで取得しないため通知候補にならない。

`20260821022637_add_configurable_notification_targets.sql` は `include_holidays` と新しいRPC署名・制約を追加する。PR #55の固定条件を表すISO曜日1〜7・08:00〜13:00の既存設定だけを土曜・日曜・祝日へ変換し、その他の対応外データが残る環境では匿名エラーで停止する。画面は移行後の保存値をそのまま表示し、旧設定専用の表示は設けない。
...
- 条件が有効で施設が1件以上あり、土曜・日曜・祝日のいずれかが選択され、施設ID、対象日、任意の `date_from` / `date_to` を満たすことを確認する。日付境界は含む。
...
通知条件は `list_notification_rules_for_matching()` から取得する。このRPCはactive会員の有効かつ施設1件以上・対象日1件以上の条件だけを対象に、条件ID、利用者ID、日付範囲、開始・終了時刻、最低時間、祝日選択、施設ID配列、ISO曜日配列だけを返し、メールアドレスを返さない。`security invoker`、`stable`、空の `search_path`、完全修飾名を使用し、既存RLSやpolicyを変更しない。実行権限は `PUBLIC`、`anon`、`authenticated` から剥奪して `service_role` だけへ付与するため、Web UIやpublishable keyからは呼び出せない。
...
| `private.line_notification_beta_allowlist` | 限定βへ許可した会員UUID | 最大20件。RLS強制、browser非公開、service-roleは直接書込不可 |
...
- 会員情報、同意履歴、通知条件は退会完了まで保持し、利用者別通知メッセージ・対象・配信結果は原則90日保持する。退会時はAuthユーザー削除とFK cascadeで利用者所有データを削除する。委託先のバックアップ、アクセスログ、セキュリティ記録、メール配信記録は各委託先の保持サイクルまたは法令上必要な期間に従う。
...
Phase 4のLINE連携解除、Phase 5の有料契約処理は各Phaseで追加する。同じメールアドレスによる再登録と、現在の認証済みJWTとは別の追加再認証の要否は引き続き運用事項として**要決定**である。退会後に委託先のバックアップ・ログ等へ残る情報は、正式プライバシーポリシーに記載した各委託先の保持サイクルまたは法令上必要な期間に従う。
...
- 将来の有料化、対象地域拡大または取扱情報追加時に必要となる規約・プライバシーポリシー改定と専門家確認
