...
| 通知条件 | `account/notifications.html` | active会員限定 | 2 | 条件一覧、作成・編集・一時停止・有効化・削除、施設、日付範囲、利用時間。曜日・時間帯はMonitoring Policyに固定 |
...
通知条件UIは、active会員本人の条件だけをRLS配下で読み込み、「登録済み n / 5件」を表示し、施設1件以上を必須検証する。会員が入力するのは施設、任意の日付範囲、利用時間であり、通知対象は土日・日本の祝日、8:00〜13:00に固定する。利用時間は60、120、180、240、300分の60分刻みで、初期値は120分とする。5件では新規作成ボタンを無効化して既存条件の削除を案内し、削除後の再取得で4件以下になれば再び有効化する。新規フォーム開始時と保存直前にも件数を確認するが、編集・有効化・一時停止・削除は上限の影響を受けない。
...
現行のMonitoring Policyでは、直近15日間の土日・日本の祝日、8:00〜13:00について、スクレイパーが連続60分以上の空き候補を生成する。会員向け通知条件も同じ曜日・時間帯へ固定し、取得範囲と通知対象を一致させる。

祝日は祝日専用条件ではなく実際の日付のISO曜日で判定する。平日の祝日も照合できるよう、固定条件の保存時はISO曜日1〜7をすべて登録する。通常の平日はMonitoring Policyで取得しないため、現行運用では通知候補にならない。

DBの既存データ構造とRPC署名は将来拡張との互換性のため維持する。固定化前の曜日・時間帯またはUI非対応の最低連続時間を持つ条件は自動変更せず、一覧に旧設定と移行案内を表示する。編集・再保存時に現行の固定条件へ更新し、停止中の旧条件は再保存するまで有効化できない。
