...
> 2026-08-28更新: 本書のメール主導フローはPhase 1実装時の履歴です。現在仕様はLINE Loginを登録・ログインの主導線、メールを任意の予備ログイン手段とし、LINE登録者のメール通知を初期OFFとします。現在仕様の正は `SERVICE_SPECIFICATION.md` を参照してください。
...
| 法務ページ | 正式版 `2026-08-21`、運営者、問い合わせ先、保持・削除、改定時の再同意を公開する | 2026-08-21 |
...
| 利用規約 | `legal/terms.html` | 公開 | 正式版 `2026-08-21`。サービス条件、免責、改定、運営者、問い合わせ |
| プライバシーポリシー | `legal/privacy.html` | 公開 | 正式版 `2026-08-21`。取得項目、目的、委託、第三者提供、保持・削除、開示等請求 |
...
- プライバシーポリシーはHTMLとGit履歴で版管理し、利用規約だけをDBの明示同意対象とする。プライバシーポリシーの重要な変更はサービス上または登録メールで案内し、利用規約も変わる場合は新規約版への再同意を求める。
...
- 開発用初期版は `2026-08-04-draft`。正式版 `2026-08-21` を追加してcurrentを切り替え、active会員を `pending_terms` へ移行して再同意を求める。旧版の同意履歴は保持する。
...
現行の退会実装ではAuthユーザーのhard deleteに伴うFK cascadeで同意履歴も削除し、2026-08-20のproduction acceptanceで削除を確認した。委託先のバックアップ、アクセスログ、セキュリティ記録およびメール配信記録は、各委託先の保持サイクルまたは法令上必要な期間残り得ることを正式プライバシーポリシーへ明記する。
...
正式プライバシーポリシーでは、退会完了時に本サービスDBの利用者所有データを
削除し、委託先のバックアップ・アクセスログ・セキュリティ記録・メール配信記録は
各委託先の保持サイクルまたは法令上必要な期間残り得ることを明記する。

同じメールアドレスによる再登録、現在の認証済みJWTに加えて追加再認証を要求するか、
監査・バックアップの運用値は継続して管理し、変更時はRunbookと必要に応じて
プライバシーポリシーへ反映する。
...
正式版 `2026-08-21` は新しい行として追加し、同じmigrationで旧版の `is_current` をfalse、新版をtrueへ切り替え、active会員を `pending_terms` へ移行して再同意を求める。既存の同意履歴と利用者所有データは削除しない。

## 20. 段階的な実装手順

### Step 0: 要決定事項のうち実装前提を確定

Supabase採用、メールのマジックリンク認証、GitHub Pages継続、PKCE、Phase 1の機能境界は2026-08-04に決定済みである。規約・プライバシー本文、運営者、問い合わせ先、重要改定時の再同意、退会時削除は2026-08-21までに決定した。環境分離、バックアップ要件等は引き続き決定し、必要ならADRへ記録する。
...
- 将来の有料化、対象地域拡大または取扱情報追加時の専門家確認
- 委託先または法令変更時の規約・プライバシーポリシー更新
- IPアドレス・User-Agentはサービス提供者の安全管理ログに残り得るが、Phase 1の同意証跡には追加しない
- 表示名は現行サービスでは収集しない
